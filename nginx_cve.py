...
def check_cve_2024_27956(url, collector):
    ...
    if status == 200 and re.search(r"@|root|wordpress|mysql", body, re.IGNORECASE):
        ...
        # Plugin existe mas sem indicador SQL — requer validação manual
        collector.add(url, cve, "POSSIBLY_VULNERABLE",
                      detail="csv.php acessível — validar SQLi manualmente",
...
def main():
    ...
    try:
        ...
        check_cve_2023_44487(url, collector, server_info=f"nginx/{nginx_ver}")
